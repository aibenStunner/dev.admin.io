...
if (isset($_SESSION['partial_logout'])) {
    Message::success(
        __('You were logged out from one server, to logout completely from GodsEye Admin, you need to logout from all servers.')
    )->display();
...
if ($server > 0 || count($cfg['Servers']) > 1
    ...
    if ($cfg['DBG']['demo']) {
        ...
        echo '<h2>' , __('GodsEye Admin Demo Server') , '</h2>';
...
echo '<h2>Godseye Admin</h2>';
echo '<h2>Manage records of parent and children, as well as camera configurations.</h2>';
echo '<ul>';
$class = null;
...
Core::printListItem(
    ...
    __('Version:') . ' <span class="version">1.0.0</span>',
    'li_pma_version',
    null,
    null,
    null,
...
Core::printListItem(
    __('View Parents'),
    'li_ge_home',
    Core::linkURL('sql.php?server=1&db=new_schema&table=parent&pos=0'),
    null,
    '_self'
);
...
Core::printListItem(
    __('View Wards'),
    'li_ge_home',
    Core::linkURL('sql.php?server=1&db=new_schema&table=ward&pos=0'),
    null,
    '_self'
);
...
Core::printListItem(
    __('View Classes'),
    'li_ge_home',
    Core::linkURL('sql.php?server=1&db=new_schema&table=class&pos=0'),
    null,
    '_self'
);
...
Core::printListItem(
    __('View Teachers'),
    'li_ge_home',
    Core::linkURL('sql.php?server=1&db=new_schema&table=teacher&pos=0'),
    null,
    '_self'
);
